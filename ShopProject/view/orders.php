...
if ($_SESSION["user"]->role->name != "admin" && $_SESSION["user"]->role->name != "manager")
    header("Location: index.php");
?>
        ...
        <table class="table table-bordered">
            <thead>
                <tr>
                    ...
                    <th scope="col" class="color-custom1">User</th>
                ...
                foreach ($orders as $order) {
                    ...
                    <tr>
                        ...
                        <td class="color-custom4">
                            ...
                            switch (strtolower($order->status->name)) {
                                case "pending":
                                    echo "<button onclick='ApproveOrder($order->id, this);' class='btn btn-primary order-action valid-border valid-bg my-2'>Approve</button>
                                    <br><button onclick='RejectOrder($order->id, this);' class='btn btn-primary order-action invalid-border invalid-bg my-2'>Reject</button>";
                                    break;
                                case "rejected":
                                    echo "<span class='invalid-color'>" . $order->status->name . "</span>";
                                    break;
